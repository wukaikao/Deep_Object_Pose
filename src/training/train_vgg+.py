...
class MultipleVertexJson(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        scale = 8
        if min (img.size) / 16.0  != min (img_size)/16.0:
            scale = min (img.size)/(min (img_size)/16.0)
            # print (scale)
        ...
        w_crop = int(w_crop/16)
        h_crop = int(h_crop/16)

        print(w_crop,h_crop,img_size[1]/16,img_size[0]/16,affinities.size())
        affinities = affinities[:,h_crop:h_crop+int(img_size[1]/16),w_crop:w_crop+int(img_size[0]/16)]
        beliefs = beliefs[:,h_crop:h_crop+int(img_size[1]/16),w_crop:w_crop+int(img_size[0]/16)]
...
#    dict_trained = torch.load(opt.net)
#
#    new_list = list(net.state_dict().keys())
#    trained_list = list(dict_trained.keys())
#    
    # print(len(new_list),len(trained_list))
#    dict_new.clear()        
#    for i in range(len(new_list)):
#        if i <= 15:
#            dict_new[ str(new_list[i]) ] = dict_trained[ trained_list[i] ]
#            # print(i,str(new_list[i]),trained_list[i])
#        else:
#            dict_new[ str(new_list[i]) ] = dict_trained[ trained_list[i+6] ]
#            # print(i,str(new_list[i]),trained_list[i+6])
#    new_list = list(net.state_dict().keys())
#    # for i in range(len(new_list)):
#    #     print(i,new_list[i])
...
def _runnetwork(epoch, loader, train=True):
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        for l in output_belief: #output, each belief map layers. 
            print(l.size(),target_belief.size())
            if loss is None:
